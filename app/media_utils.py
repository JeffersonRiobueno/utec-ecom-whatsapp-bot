...
def transcribe_audio(base64_audio: str, provider: str = "gemini") -> str:
    ...
    if provider.lower() == "gemini":
        return _transcribe_audio_gemini(audio_data)
    elif provider.lower() == "openai":
        return _transcribe_audio_openai(audio_data)
    else:
        logger.warning(f"Provider {provider} no soporta transcripción de audio. Usando Gemini por defecto.")
        return _transcribe_audio_gemini(audio_data)

def _transcribe_audio_gemini(audio_data: bytes) -> str:
    try:
        ...
        logger.info(f"Transcripción Gemini obtenida: '{transcribed}'")
...
def extract_text_from_image(base64_image: str, mimetype: str, provider: str = "gemini") -> str:
    ...
    if provider.lower() == "gemini":
        return _extract_text_from_image_gemini(base64_image, mimetype)
    elif provider.lower() == "openai":
        return _extract_text_from_image_openai(base64_image, mimetype)
    else:
        logger.warning(f"Provider {provider} no soporta OCR. Usando Gemini por defecto.")
        return _extract_text_from_image_gemini(base64_image, mimetype)

def _extract_text_from_image_gemini(base64_image: str, mimetype: str) -> str:
    try:
        llm = get_gemini_llm()
        ...
        logger.info(f"Texto extraído Gemini: '{extracted}'")
        ...
        logger.error(f"Error en OCR Gemini: {e}")
